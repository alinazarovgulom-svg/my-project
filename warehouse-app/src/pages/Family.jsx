  ...
  if (!team) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24">
        <div className="bg-white dark:bg-slate-900 px-5 pt-14 pb-6">
          <h1 className="text-slate-900 dark:text-white text-xl font-bold">{t('teamMode')}</h1>
        </div>
        <div className="px-4 pt-8 space-y-4">
          <div className="bg-slate-100 dark:bg-slate-800/40 rounded-3xl p-8 flex flex-col items-center text-center gap-3 mb-6">
            ...
            <h2 className="text-slate-900 dark:text-white font-semibold text-lg">{t('teamMode')}</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm">Bir necha omborchi bilan real vaqtda ishlang</p>
          </div>
          ...
          <button onClick={() => setCreateModal(true)}
            ...
            <div className="text-left">
              <p className="text-slate-900 dark:text-white font-medium">{t('createTeam')}</p>
              <p className="text-slate-500 dark:text-slate-400 text-xs">Yangi jamoa yaratib, a'zolarni qo'shing</p>
            </div>
          </button>

          <button onClick={() => setJoinModal(true)}
            className="w-full flex items-center gap-3 bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/40 rounded-2xl p-5 active:scale-95 transition-all">
            <div className="w-10 h-10 rounded-xl bg-slate-200 dark:bg-slate-700/60 flex items-center justify-center">
              <LogIn size={20} className="text-slate-700 dark:text-slate-300" />
            </div>
            <div className="text-left">
              <p className="text-slate-900 dark:text-white font-medium">{t('joinTeam')}</p>
              <p className="text-slate-500 dark:text-slate-400 text-xs">Taklif kodi orqali jamoaga kirish</p>
            </div>
          </button>
        </div>
      ...
      <div className="px-4 py-4 space-y-4">
        <div className="bg-gradient-to-br from-primary-600/20 to-emerald-600/10 border border-primary-500/20 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-slate-900 dark:text-white font-bold text-lg">{team.name}</p>
              <p className="text-slate-500 dark:text-slate-400 text-sm">{team.members?.length || 0} a'zo</p>
            </div>
          ...
          <div className="bg-slate-100/80 dark:bg-slate-900/50 rounded-xl p-3 flex items-center justify-between">
            <div>
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-0.5">{t('inviteCode')}</p>
              <p className="text-slate-900 dark:text-white font-mono font-bold tracking-wider">{teamId}</p>
            </div>
        ...
        <div>
          <h2 className="text-slate-900 dark:text-white font-semibold mb-3">{t('members')}</h2>
          <div className="space-y-2">
            {team.members?.map(m => (
              <div key={m.userId} className="bg-slate-100 dark:bg-slate-800/60 rounded-xl px-4 py-3.5 border border-slate-200 dark:border-slate-700/30">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-900 dark:text-white font-semibold text-sm">
                      {(m.fullName || m.username || '?')[0].toUpperCase()}
                    </div>
                    <div>
                      <p className="text-slate-900 dark:text-white text-sm font-medium">{m.fullName || m.username}</p>
                      <div className="flex items-center gap-1 mt-0.5">
                        {roleIcon(m.role)}
                        <span className="text-slate-500 dark:text-slate-400 text-xs">{t(m.role)}</span>
                  ...
                  {userRole === 'admin' && m.userId !== user.id && (
                    <div className="flex gap-2">
                      <select value={m.role} onChange={e => handleRoleChange(m.userId, e.target.value)}
                        className="bg-slate-200 dark:bg-slate-700 border border-slate-300 dark:border-slate-600/50 rounded-lg px-2 py-1 text-slate-700 dark:text-slate-300 text-xs focus:outline-none">
