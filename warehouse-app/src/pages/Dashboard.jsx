...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-24">
      {/* Header */}
      <div className="bg-gradient-to-b from-slate-900 to-slate-950 px-5 pt-14 pb-6">
        <div className="flex items-center justify-between mb-1">
          <div>
            <p className="text-slate-400 text-sm">{t('hello')}</p>
            <h1 className="text-white text-xl font-bold">{user?.fullName || user?.username}</h1>
          </div>
      ...
      <div className="px-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/40">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-slate-400 text-xs">{t('totalProducts')}</span>
            </div>
            <p className="text-white text-2xl font-bold">{activeProducts.length}</p>
          </div>
          ...
          <div className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/40">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-slate-400 text-xs">{t('lowStock')}</span>
            </div>
          ...
          <div className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/40">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-slate-400 text-xs">{t('todayIn')}</span>
            </div>
          ...
          <div className="bg-slate-800/60 rounded-2xl p-4 border border-slate-700/40">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-slate-400 text-xs">{t('todayOut')}</span>
            </div>
        ...
        <div className="bg-gradient-to-r from-primary-600/20 to-emerald-600/20 rounded-2xl p-5 border border-primary-500/20">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-slate-400 text-sm">{t('totalValue')}</span>
          </div>
          <p className="text-white text-3xl font-bold">{fmtNum(totalValue)}</p>
          <p className="text-slate-400 text-sm">so'm</p>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-white font-semibold">{t('recentMovements')}</h2>
          ...
          {recent.length === 0 ? (
            <div className="bg-slate-800/40 rounded-2xl p-8 text-center">
              <Boxes size={32} className="text-slate-600 mx-auto mb-2" />
              <p className="text-slate-500 text-sm">{t('noMovements')}</p>
            </div>
          ) : (
            <div className="space-y-2">
              {recent.map(mv => (
                <div key={mv.id} className="bg-slate-800/60 rounded-xl px-4 py-3 flex items-center justify-between border border-slate-700/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white text-sm font-medium">{mv.productName}</p>
                      <p className="text-slate-400 text-xs">{fmtDate(mv.date)} · {mv.quantity} {mv.unit}</p>
                    </div>
                  </div>
