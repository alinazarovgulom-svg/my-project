    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24">
      <div className="bg-white dark:bg-slate-900 px-5 pt-14 pb-4">
        <div className="flex items-center justify-between mb-1">
          <h1 className="text-slate-900 dark:text-white text-xl font-bold">{t('products')}</h1>
          <div className="flex items-center gap-2">
            <button onClick={openImport}
              className="w-10 h-10 rounded-xl bg-slate-200 dark:bg-slate-700 flex items-center justify-center">
        ...
        <p className="text-slate-600 dark:text-slate-500 text-xs">{products.length} ta mahsulot</p>
      </div>
      ...
      <div className="px-4 py-3">
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            <Package size={48} className="text-slate-400 dark:text-slate-700 mb-3" />
            ...
            {!search && (
              <div className="flex gap-3 mt-4">
                ...
                <button onClick={openImport}
                  className="px-5 py-2.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-sm font-medium flex items-center gap-2">
              ...
              <div key={p.id}>
                {idx === pinnedCount && pinnedCount > 0 && pinnedCount < filtered.length && (
                  <div className="flex items-center gap-2 my-2 px-1">
                    ...
                    <span className="text-slate-400 dark:text-slate-600 text-xs">Boshqalar</span>
                    <div className="flex-1 h-px bg-slate-200 dark:bg-slate-700/50" />
                  </div>
                )}
                <SwipeableRow onEdit={() => openEdit(p)} onDelete={() => handleDelete(p.id)}>
                  <div className={`bg-slate-100 dark:bg-slate-800/60 rounded-xl px-4 py-3.5 border ${pinned.includes(p.id) ? 'border-amber-500/25' : 'border-slate-200 dark:border-slate-700/30'}`}>
                    <div className="flex items-center gap-3">
                      {p.image
                        ? <img src={p.image} alt={p.name} className="w-12 h-12 rounded-xl object-cover flex-shrink-0" />
                        : <div className="w-12 h-12 rounded-xl bg-slate-200 dark:bg-slate-700/60 flex items-center justify-center flex-shrink-0">
                            <Package size={20} className="text-slate-400 dark:text-slate-500" />
                          </div>
                      }
                      <div className="flex-1 min-w-0 flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          <p className="text-slate-900 dark:text-white font-medium text-sm truncate">{p.name}</p>
                          <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">{p.category} · {p.unit}</p>
                          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            {p.minStock > 0 && <span className="text-slate-600 dark:text-slate-500 text-xs">Min: {p.minStock} {p.unit}</span>}
                            {p.location && (
                        ...
                        <div className="flex flex-col items-end gap-1.5 ml-3">
                          <button
                            ...
                            <Star size={15} className={pinned.includes(p.id) ? 'text-amber-400 fill-amber-400' : 'text-slate-400 dark:text-slate-600'} />
                          </button>
                          <p className="text-primary-400 text-sm font-semibold">{fmtNum(p.purchasePrice)} so'm</p>
                          {p.salePrice > 0 && <p className="text-slate-500 dark:text-slate-400 text-xs">{fmtNum(p.salePrice)} so'm</p>}
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? t('editProduct') : t('addProduct')}>
        <div className="space-y-3 pb-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-slate-500 dark:text-slate-400 text-xs mb-1 block">{t('purchasePrice')}</label>
              <input type="number" value={form.purchasePrice} onChange={set('purchasePrice')} placeholder="0"
                className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 rounded-xl px-4 py-3 text-slate-900 dark:text-white text-sm placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-primary-500/40" />
            </div>
            <div>
              <label className="text-slate-500 dark:text-slate-400 text-xs mb-1 block">{t('salePrice')}</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-slate-500 dark:text-slate-400 text-xs mb-1 block">{t('minStock')}</label>
              <input type="number" value={form.minStock} onChange={set('minStock')} placeholder="0"
                className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 rounded-xl px-4 py-3 text-slate-900 dark:text-white text-sm placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-primary-500/40" />
            </div>
            <div>
              <label className="text-slate-500 dark:text-slate-400 text-xs mb-1 block">{t('barcode')}</label>
          ...
          <div className="relative">
            ...
            <input value={form.location} onChange={set('location')} placeholder="Joylashuv: A-3 javon, 2-qator..."
              className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 rounded-xl pl-10 pr-4 py-3 text-slate-900 dark:text-white text-sm placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-amber-500/40" />
          </div>
          ...
          {form.image ? (
            ...
            <button onClick={() => imageRef.current?.click()}
              className="w-full flex items-center gap-3 bg-slate-100 dark:bg-slate-800/60 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl px-4 py-3 active:scale-95 transition-all">
      ...
      <Modal open={importModal} onClose={() => setImportModal(false)} title="Excel import">
        <div className="space-y-4 pb-4">
          ...
          <button onClick={downloadTemplate}
            className="w-full flex items-center gap-3 bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/40 rounded-xl px-4 py-3.5 active:scale-95 transition-all">
            ...
            <div className="text-left">
              <p className="text-slate-900 dark:text-white text-sm font-medium">Shablon yuklab olish</p>
              <p className="text-slate-500 dark:text-slate-400 text-xs">Excel shablonni oching va to'ldiring</p>
            </div>
          </button>
          ...
          {!importResult && !importing && (
            <button onClick={() => fileRef.current?.click()}
              ...
              <div className="text-left">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-xs">.xlsx, .xls yoki .csv</p>
              </div>
            </button>
          )}
          ...
          {importing && (
            <div className="flex items-center justify-center gap-3 py-6">
              ...
              <p className="text-slate-700 dark:text-slate-300 text-sm">O'qilmoqda...</p>
            </div>
          )}
            ...
            <div className="space-y-3">
              <div className="bg-primary-500/10 border border-primary-500/20 rounded-xl px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-900 dark:text-white text-sm">Tayyor mahsulotlar</span>
                </div>
                ...
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl overflow-hidden">
                  <button onClick={() => setShowErrors(s => !s)}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-slate-900 dark:text-white text-sm">Xatolar</span>
                    </div>
                    <div className="flex items-center gap-2">
              ...
              {importResult.products.length > 0 && (
                <div className="bg-slate-100 dark:bg-slate-800/40 rounded-xl overflow-hidden">
                  <p className="text-slate-500 dark:text-slate-400 text-xs px-4 py-2 border-b border-slate-200 dark:border-slate-700/50">Ko'rinish (dastlabki 3 ta)</p>
                  {importResult.products.slice(0, 3).map((p, i) => (
                    <div key={i} className={`px-4 py-2.5 flex justify-between items-center ${i > 0 ? 'border-t border-slate-200 dark:border-slate-700/30' : ''}`}>
                      <div>
                        <p className="text-slate-900 dark:text-white text-sm">{p.name}</p>
                        <p className="text-slate-500 dark:text-slate-400 text-xs">{p.category} · {p.unit}</p>
                      </div>
                      <p className="text-primary-400 text-sm font-medium">{fmtNum(p.purchasePrice)} so'm</p>
                    </div>
                  ))}
                  {importResult.products.length > 3 && (
                    <p className="text-slate-500 text-xs px-4 py-2 border-t border-slate-200 dark:border-slate-700/30">
              ...
              {importResult.products.length > 0 ? (
                ...
                <button onClick={() => { setImportResult(null); fileRef.current?.click() }}
                  className="w-full bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 py-3.5 rounded-xl text-sm">
                  Boshqa fayl tanlash
                </button>
              )}
            </div>
          ...
          {importDone && importResult && (
            <div className="space-y-3">
              <div className="flex flex-col items-center py-6 gap-3">
                ...
                <p className="text-slate-900 dark:text-white font-semibold text-lg">Import tugadi!</p>
                <div className="flex gap-4 text-center">
                  <div>
                    <p className="text-primary-400 font-bold text-2xl">{importResult.imported}</p>
                    <p className="text-slate-500 dark:text-slate-400 text-xs">qo'shildi</p>
                  </div>
                  {importResult.duplicates > 0 && (
                    <div>
                      <p className="text-slate-500 dark:text-slate-400 font-bold text-2xl">{importResult.duplicates}</p>
                      <p className="text-slate-500 dark:text-slate-400 text-xs">takror (o'tkazildi)</p>
                    </div>
                  )}
                  {importResult.errors.length > 0 && (
                    <div>
                      <p className="text-amber-400 font-bold text-2xl">{importResult.errors.length}</p>
                      <p className="text-slate-500 dark:text-slate-400 text-xs">xato</p>
                    </div>
                  )}
                </div>
              </div>
