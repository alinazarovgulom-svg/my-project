...
export default function Onboarding({ onDone }) {
  ...
  return (
    <div className={`fixed inset-0 z-[998] bg-slate-950 flex flex-col items-center justify-center transition-opacity duration-400 ${visible ? 'opacity-100' : 'opacity-0'}`}>
      {/* Background glow */}
      ...
      <div className="relative flex flex-col items-center gap-8 px-8 w-full max-w-sm">
        ...
        <div className={`flex flex-col items-center gap-3 transition-all duration-700 ${phase >= 1 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="text-center">
            <h1 className="text-3xl font-bold text-white tracking-tight">OmborBek</h1>
        ...
        <div className={`w-full space-y-2.5 transition-all duration-700 delay-200 ${phase >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          {features.map(({ icon: Icon, label, desc }, i) => (
            <div key={i}
              className="flex items-center gap-3 bg-slate-800/60 rounded-2xl px-4 py-3 border border-slate-700/50"
